...
def plot_hist(fname, data, labels, window_size, blksize):
    ...
    plt.xlabel(r"time (s)", fontsize=11)
    
    if args.blksize is None:
        plt.ylabel(r"blocks/s", fontsize=11)
    else:
        plt.ylabel(r"tx/s", fontsize=11)

    colors = list(mcolors.TABLEAU_COLORS)
    offsets = [0.22,0,-0.22,-0.09, -0.09]
    for i, (x, y, reconfig_x, avg_tx_sec, total, total_time) in enumerate(data):
        ...
        plt.plot(reconfig_x, reconfig_y, 'D', color=color, markersize=9)
        
        plt.axhline(y=(total / total_time), color=color, linestyle='-', linewidth=1)
        plt.text(143, avg_tx_sec+offsets[i], 'Average=' + str(round(float(avg_tx_sec), 3)), color=color, ha='left', va='bottom', bbox=dict(facecolor='white', alpha=0.75), fontsize=12)
