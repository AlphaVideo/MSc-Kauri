...
def plot_hist(fname, data, labels, window_size, blksize):
    ...
    for i, (x, y, reconfig_x, avg_tx_sec, total, total_time) in enumerate(data):
        ...
        plt.text(400, avg_tx_sec+offsets[i], 'Average=' + str(round(float(avg_tx_sec), 3)), color=color, ha='left', va='bottom', bbox=dict(facecolor='white', alpha=0.75), fontsize=12)
